...
# Generate year-based listing for subjects ending in 8
if ($PDFTemplateCode -match "8$") {

	Write-Host -ForegroundColor Yellow "Updating year-based listing"

    $CurrentHTML = Get-Content -Encoding UTF8 $PapersFile -Raw

    # Remove any existing papers-by-year table
    $CurrentHTML = [regex]::Replace(
        $CurrentHTML,
        '(?s)<table\b[^>]*\bid\s*=\s*"papers-by-year"[^>]*>.*?</table>\s*',
        ''
    )
    ...
    $Years = $PapersFormatted |
        ForEach-Object {
            if ($_ -match "\b((?:19|20)[0-9]{2})\b") {
                $Matches[1]
            }
        } |
    ...
    foreach ($Year in $Years) {
        ...
        foreach ($PaperName in $YearPapers) {

            # Split:
            # Abbotsleigh 2002 w. sol
            # into:
            # Abbotsleigh
            # 2002
            # w. sol

            if ($PaperName -match "^(.*?)\s+((?:19|20)[0-9]{2})(?:\s+(.*))?$") {
                ...
                if ($Suffix) {
                    $DisplayName = "$PaperYear $SchoolName $Suffix"
                }
                else {
                    $DisplayName = "$PaperYear $SchoolName"
                }

                $YearLinks += "<a href=`"#v`" onClick=`"pdf(this, $PDFTemplateCode)`" data-title=`"$PaperName`">$DisplayName</a>"
            }
        }

        if ($YearLinks.Count -gt 0) {

            $YearTableRows += @"
<tr><td><details><summary>$Year</summary><br />
<span class="content">
$($YearLinks -join "<br />`r`n")
</span></details></td></tr>
"@
        }
    }
    ...
    $YearTableHTML = @"
<table class="listing" id="papers-by-year" style="display:none;">
<tbody>
$($YearTableRows -join "`r`n")
</tbody>
</table>
"@

    # Append the year table after the existing papers-by-school table
    $CurrentHTML = gc -Encoding UTF8 $PapersFile -Raw

    $TableEnd = $CurrentHTML.IndexOf("</table>", $CurrentHTML.IndexOf('id="papers-by-school"'))

    if ($TableEnd -ge 0) {

        $InsertPosition = $TableEnd + "</table>".Length

        $NewHTML = `
            $CurrentHTML.Substring(0, $InsertPosition) +
            "`r`n" +
            $YearTableHTML +
            $CurrentHTML.Substring($InsertPosition)

        Set-Content -Encoding UTF8 $PapersFile -Value $NewHTML
    }
}
